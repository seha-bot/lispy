...
struct Context {
    ...
    std::string tip(ast::TypeId t) {
        if (auto a = std::get_if<ast::TypeReference>(&ts.read(t))) {
            auto name =
                std::visit([](auto& entity) { return static_cast<std::string>(entity.name); },
    ...
    struct Visitor {
        ...
        std::expected<ast::TypeId, Error> operator()(ast::Call const& call) {
            ...
            for (auto& arg : call.arguments | std::views::reverse) {
                ...
                callee_type_calculated =
                    ctx.ts.store(ast::TypeArrow{*arg_type, callee_type_calculated});
            }

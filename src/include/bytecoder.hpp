...
inline void assemble(std::ostream &os, std::vector<std::string> const &atoms,
                     std::optional<InstrPtr> entry_point, std::vector<Instruction> const &instrs) {
...
inline Mnemonic opcode(Bytecode const &bc, InstrPtr ip) {
  return Mnemonic(read_n(bc, ip.value(), 1));
}

...
Parser<ast::TypeId> type_parser(Context ctx) noexcept {
  ...
  auto lookup = [ctx](std::string name) {
...
Parser<ast::Expr> expr_parser(Context ctx);
...
Parser<ast::Expr> special_parser(Context ctx) {
  auto lambda_parser = [ctx](std::monostate) -> Parser<ast::Lambda> {
  ...
  auto case_parser = [ctx](std::monostate) -> Parser<ast::Case> {
...
Parser<ast::Expr> expr_parser(Context ctx) {
  ...
  auto to_entity_reference = [ctx](std::string name) -> ast::Expr {
...
Parser<ast::ShallowEntity> module_entity_parser() {
